...
#include "bcos-framework/interfaces/executor/StateInterface.h"
...
public:
    ...
    bool addressInUse(const std::string& _address) const override;
    ...
    bool accountNonemptyAndExisting(const std::string& _address) const override;

    /// Check if the address contains executable code.
    bool addressHasCode(const std::string& _address) const override;

    /// Get an account's balance.
    /// @returns 0 if the address has never been used.
    u256 balance(const std::string& _address) const override;

    /// Add some amount to balance.
    /// Will initialise the address if it has never been used.
    void addBalance(const std::string& _address, u256 const& _amount) override;
    ...
    void subBalance(const std::string& _address, u256 const& _value) override;

    /// Set the balance of @p _address to @p _value.
    /// Will instantiate the address if it has never been used.
    void setBalance(const std::string& _address, u256 const& _value) override;
     ...
     */
    void transferBalance(const std::string& _from, const std::string& _to, u256 const& _value) override;

    /// Get the root of the storage of an account.
    crypto::HashType storageRoot(const std::string& _address) const override;

    /// Get the value of a storage position of an account.
    /// @returns 0 if no account exists at that address.
    u256 storage(const std::string& _address, u256 const& _memory) override;

    /// Set the value of a storage position of an account.
    void setStorage(const std::string& _address, u256 const& _location, u256 const& _value) override;

    /// Clear the storage root hash of an account to the hash of the empty trie.
    void clearStorage(const std::string& _address) override;

    /// Create a contract at the given address (with unset code and unchanged balance).
    void createContract(const std::string& _address) override;

    /// Sets the code of the account. Must only be called during / after contract creation.
    void setCode(const std::string& _address, bytesConstRef _code) override;

    /// Delete an account (used for processing suicides). (set suicides key = 1 when use AMDB)
    void kill(const std::string& _a) override;

    /// Get the storage of an account.
    /// @note This is expensive. Don't use it unless you need to.
    /// @returns map of hashed keys to key-value pairs or empty map if no account exists at that
    /// address.
    // virtual std::map<crypto::HashType, std::pair<u256, u256>> storage(const std::string& _address) const
    // override;
    ...
    std::shared_ptr<bytes> code(const std::string& _address) const override;

    /// Get the code hash of an account.
    /// @returns EmptyHash if no account exists at that address or if there is no code
    /// associated with the address.
    crypto::HashType codeHash(const std::string& _address) const override;

    /// Get the frozen status of an account.
    /// @returns ture if the account is frozen.
    bool frozen(const std::string& _address) const override;

    /// Get the byte-size of the code of an account.
    /// @returns code(_address).size(), but utilizes CodeSizeHash.
    size_t codeSize(const std::string& _address) const override;

    /// Increament the account nonce.
    void incNonce(const std::string& _address) override;

    /// Set the account nonce.
    void setNonce(const std::string& _address, u256 const& _newNonce) override;

    /// Get the account nonce -- the number of transactions it has sent.
    /// @returns 0 if the address has never been used.
    u256 getNonce(const std::string& _address) const override;
    ...
    void setRoot(crypto::HashType const& _root) override;
    ...
    void setMemoryTableFactory(std::shared_ptr<storage::TableFactoryInterface> _tableFactory)
    {
        m_tableFactory = _tableFactory;
    }

private:
    void createAccount(const std::string& _address, u256 const& _nonce, u256 const& _amount = u256(0));
    std::shared_ptr<storage::TableInterface> getTable(const std::string& _address) const;
