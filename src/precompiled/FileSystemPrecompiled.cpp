...
FileSystemPrecompiled::FileSystemPrecompiled(crypto::Hash::Ptr _hashImpl)
    : Precompiled(_hashImpl) {
  name2Selector[FILE_SYSTEM_METHOD_LIST] =
      getFuncSelector(FILE_SYSTEM_METHOD_LIST, _hashImpl);
  name2Selector[FILE_SYSTEM_METHOD_MKDIR] =
      getFuncSelector(FILE_SYSTEM_METHOD_MKDIR, _hashImpl);
}

std::string FileSystemPrecompiled::toString() { return "FileSystem"; }

std::shared_ptr<PrecompiledExecResult>
FileSystemPrecompiled::call(std::shared_ptr<executor::BlockContext> _context,
                            bytesConstRef _param, const std::string &,
                            const std::string &, int64_t _remainGas) {
  uint32_t func = getParamFunc(_param);
  bytesConstRef data = getParamData(_param);
  PRECOMPILED_LOG(DEBUG) << LOG_BADGE("FileSystemPrecompiled")
                         << LOG_DESC("call") << LOG_KV("func", func);
...
void FileSystemPrecompiled::makeDir(
    const std::shared_ptr<executor::BlockContext> &_context,
    bytesConstRef &data, std::shared_ptr<PrecompiledExecResult> callResult,
    const PrecompiledGas::Ptr &gasPricer) {
  // mkdir(string)
  std::string absolutePath;
  auto codec = std::make_shared<PrecompiledCodec>(_context->hashHandler(),
                                                  _context->isWasm());
  ...
  if (table) {
    ...
    callResult->setExecResult(
        codec->encode(s256((int)CODE_FILE_ALREADY_EXIST)));
  } else {
    PRECOMPILED_LOG(TRACE) << LOG_BADGE("FileSystemPrecompiled")
                           << LOG_DESC(
                                  "directory not exists, recursive build dir")
                           << LOG_KV("path", absolutePath);
    auto buildResult =
        recursiveBuildDir(_context->getTableFactory(), absolutePath);
  ...
  if (table) {
    ...
    if (type == FS_TYPE_DIR) {
      ...
      for (size_t i = 0; i < fileNameList.size(); ++i) {
        ...
        if (!entry) {
          PRECOMPILED_LOG(WARNING) << LOG_BADGE("FileSystemPrecompiled")
                                   << LOG_DESC("getRows return null entry")
                                   << LOG_KV("fileName", fileName);
          continue;
        }
        Json::Value file;
    ...
    PRECOMPILED_LOG(ERROR) << LOG_BADGE("FileSystemPrecompiled")
                           ...
                           << LOG_KV("path", absolutePath);
    getErrorCodeOut(callResult->mutableExecResult(), CODE_FILE_NOT_EXIST,
                    codec);
  }
}
